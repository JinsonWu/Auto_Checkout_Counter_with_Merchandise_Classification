...
def train(epoch):
    ...
    if (epoch % 10 == 0):
        ...
        save_checkpoint(checkpoint, filename="./res18/checkpoint_loss"+"_"+str(epoch)+".pth")
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        save_name = "./res18/checkpoint_acc"+"_"+str(epoch)+".pth"
...
model = ResNet18(img_channel=3, num_classes=18)

optimizer = Adam(model.parameters(), lr = 0.125)

if (load_model):
    print("=> Loading Checkpoint")
    checkpoint = torch.load("./pre_2/checkpoint_acc_91.pth")
...
if torch.cuda.is_available():
    ...
    critic = torch.nn.DataParallel(critic, device_ids=[0, 1, 2, 3])
    model = model.cuda()
    critic = critic.cuda()
